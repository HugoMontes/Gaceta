...
import bo.gaceta.rcb.bll.taEmpresaBll;
import bo.gaceta.rcb.bll.taPublicacionesContentBll;
...
@Named(value = "modificacionBeanSA")
@ViewScoped
public class ModificacionBeanSA implements Serializable {
    ...
    public void cargaMunipDoc(ValueChangeEvent evento) {
        ...
        try {
            ...
            Logger.getLogger(ModificacionBeanSA.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    public void registraPublicacion() {
        if (aceptacion) {
            try {
                ...
                String TituloPublica = "Modificación de Constitución de Sociedad de Anónima" + usuarioSesion.getSociedadConstituye();
                publicacionRegistro.setTituloPublica(TituloPublica);
                String resumenPublica = "Modificación de Constitución de la Sociedad:" + usuarioSesion.getSociedadConstituye();
    ...
    public void generaPreview() {
        ...
        txtVistaPrevia = "PUBLICACION DEL TESTIMONIO DE MODIFICACIÓN DE SOCIEDAD DE ANONIMA" + "\n"
                + " -----------------------------------------------------------------------------------\n"
                + "INTRODUCCION NOTARIAL" + "\n"
                + introNot + "\n"
                + "DESCRIPCION TEXTUAL DE LA MODIFICACIÓN CONFORME A LA CLAUSULA RESPECTIVA" + "\n"
                + descr1 + "\n"
                + "DATOS DEL ACTA" + "\n"
                + "Tipo de Reunion:" + tipoJunta + ", llevada a cabo el " + df.format(fecJunta) + " en el municipio:" + distOrigen + " del Departamento de:" + txtDepartamento + "\n"
                + "Orden del Día:" + "\n"
                + "--------------" + "\n"
                + txtOrdenCompleto + "\n"
                + "CONCLUSION NOTARIAL" + "\n"
